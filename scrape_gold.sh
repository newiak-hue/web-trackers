...
URL="https://kitco.com/charts/gold/"
TMPFILE="/tmp/scrape_gold.html"
BACKUP_FILE="/tmp/gold_backup.csv"
...
if [[ -s "$BACKUP_FILE" ]]; then
  TMP_INSERT="/tmp/gold_backup_processing.csv"
  cp "$BACKUP_FILE" "$TMP_INSERT"

  while IFS=',' read -r scrape_time price_val high_val low_val url_val; do
    sql="INSERT INTO gold_price (scrape_time, price_usd, high_24, low_24, notes) VALUES ('$scrape_time', $price_val, ${high_val:-NULL}, ${low_val:-NULL}, '$url_val');"
